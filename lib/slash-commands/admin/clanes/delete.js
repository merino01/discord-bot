...
export async function deleteClan (interaction, clanId) {
	...
	if (clanesConfig.archive) {
		const archived = await archiveTextChannels({ textChannelIds })
		if (archived === "error") {
			return {
	...
	logClanCommandToChannel({
		...
		command: "/admin clanes delete",
		fields: [
			{ name: "Clan", value: clan.name }
		],
		color: COLORS.red
	})
	return {
...
async function archiveTextChannels ({ textChannelIds }) {
	const clanesConfig = await ClanesConfig.get()
