...
export async function checkMessageFormat (message) {
	const { content, channel } = message
	const channelFormat = await MessageFormatsService.findOneByChannel(channel.id)
	...
	try {
		...
		if (regex.test(content)) {
			if (timeout) {
				await PendingMessagesService.insertOne({
					channel_id: channel.id,
					message_id: message.id,
					delete_at: addSecondsToDate(timeout)
				})
			}
			return
		}
